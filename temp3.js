...
for (let line of lines) {
  if (line && line.includes('cmd-scene-explorer')) {
    try {
      ...
      if (obj.content && obj.content.includes('<div class="cmd-scene-explorer" id="cmd-scene-explorer">')) {
        const start = obj.content.indexOf('<div class="cmd-scene-explorer" id="cmd-scene-explorer">');
        if (start > -1) {
          const end = obj.content.indexOf('  const cmdTimelineHTML', start);
          fs.writeFileSync('original_scene_explorer.txt', obj.content.substring(start, end));
          break;
        }
      }
    } catch(e){}
